$(document).ready(function (event) {
    ...
    $(document).off("click").on("click", '.removebtn', function (event) {
        event.stopImmediatePropagation();
        console.log('will remove this order...')
        console.log(event.target.id);
        var movieId = event.target.id;
        RemoveFromCart(movieId)
        $('#table' + movieId).remove();

        /*if($('#cartlist').length == 1){
            console.log('cart length is one, so making table hidden')
    ...
    function RemoveFromCart(movieId){
        ...
        $.ajax({
            type: 'POST',
            url: 'http://127.0.0.1:3999/api/g/billing/cart/delete',
            ...
            data: JSON.stringify({email: Cookies.get('email'), movieId: movieId}),
            success: function (data, textStatus, jqXHR) {
                console.log("Success in removing from cart")
            ...
            success: function (data, textStatus, xhr) {
                ...
                if (xhr.status == 204) {
                    ...
                    if (resultCode == 33) {
                        ...
                        console.log("Shopping cart item removed successfully");
