...
import edu.uci.ics.khefner.service.api_gateway.models.JsonErrorModel;
...
import static edu.uci.ics.khefner.service.api_gateway.GatewayService.ANSI_RED;
import static edu.uci.ics.khefner.service.api_gateway.GatewayService.ANSI_RESET;
import static edu.uci.ics.khefner.service.api_gateway.utilities.ResultCodes.*;
import static edu.uci.ics.khefner.service.api_gateway.utilities.HTTPStatusCodes.*;
    ...
    public static Response returnInvalidRequest(ModelValidationException e){//, Class modelType) {
        try {
            ...
            if (e.getCause() instanceof JsonMappingException) {
                ...
                responseModel = new JsonErrorModel(-3, "JSON Parse Exception.");
            } else {
                //object = constructor.newInstance(INTERNAL_SERVER_ERROR);
                //resultCode = INTERNAL_SERVER_ERROR;
                responseModel = new JsonErrorModel(-1, "Internal server error..");
            }

            if(responseModel.getResultCode() == -1){return Response.status(Status.INTERNAL_SERVER_ERROR).entity(responseModel).build();}
